(async function() {
  ...
  // DBから設定（金額など）を取得。失敗時はデフォルト値を使用
  let s = { PRICE_KEY_RING: 1500, PRICE_CHARM: 2800, PRICE_TEMPLATE: 500, PRICE_TAX: 0.1, PRICE_POSTAGE: 380 };
  try {
  ...
  style.innerHTML = `
    ...
    .kt-req, .kt-opt { color: white; font-size: 0.7rem; padding: 2px 6px; border-radius: 4px; margin-left: 8px; vertical-align: middle; }
    .kt-req { background: #e11d48; } .kt-opt { background: #9ca3af; }
    ...
    .kt-radio-label { display: flex; align-items: center; gap: 8px; cursor: pointer; font-size: 0.95rem; font-weight: 500; background: #fdf2f8; padding: 10px 16px; border-radius: 8px; border: 2px solid transparent; flex: 1; min-width: 200px; }
    ...
    .kt-submit-btn { width: 100%; padding: 20px; background: linear-gradient(135deg, #f43f5e, #e11d48); color: #fff; border: none; border-radius: 50px; font-size: 1.2rem; font-weight: 800; cursor: pointer; }
      ...
      <form id="ar-embed-form">
        ...
        <div class="kt-form-body">
          ...
          <div class="kt-form-group" id="kt-file-group">
            ...
            <div class="kt-file-area"><input type="file" id="kt-file" class="kt-input" accept="image/*,video/mp4,video/webm" style="border: none; background: transparent; padding: 0;" required /><div id="kt-file-status" style="margin-top:12px; color:#be123c; font-weight:bold; display:none;"></div></div>
          </div>
  ...
  const updateFormState = () => {
    ...
    const optionPrice = imageType === 'テンプレート' ? s.PRICE_TEMPLATE : 0;
    const subTotal = basePrice + optionPrice + s.PRICE_POSTAGE;
